...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (uLineStyle > 0.5) {
    ...
    if (uLineStyle < 1.5) { lineX *= step(vy, dashDuty); lineY *= step(vx, dashDuty); }
      ...
      float dotRepeat = 6.0, dotWidth = 0.18;
      lineX *= 1.0 - smoothstep(dotWidth, dotWidth + fwidth(gridUV.y * dotRepeat), abs(fract(gridUV.y * dotRepeat) - 0.5));
      lineY *= 1.0 - smoothstep(dotWidth, dotWidth + fwidth(gridUV.x * dotRepeat), abs(fract(gridUV.x * dotRepeat) - 0.5));
    }
  }
  ...
  float aax2 = wx2, aay2 = wy2;
  float lineX2 = 1.0 - smoothstep(tx2, tx2 + aax2, ax2);
  float lineY2 = 1.0 - smoothstep(ty2, ty2 + aay2, ay2);
  if (uLineStyle > 0.5) {
    float dashRepeat2 = 4.0, dashDuty2 = 0.5;
    if (uLineStyle < 1.5) { lineX2 *= step(fract(gridUV2.y * dashRepeat2), dashDuty2); lineY2 *= step(fract(gridUV2.x * dashRepeat2), dashDuty2); }
    else {
      float dotRepeat2 = 6.0, dotWidth2 = 0.18;
      lineX2 *= 1.0 - smoothstep(dotWidth2, dotWidth2 + fwidth(gridUV2.y * dotRepeat2), abs(fract(gridUV2.y * dotRepeat2) - 0.5));
      lineY2 *= 1.0 - smoothstep(dotWidth2, dotWidth2 + fwidth(gridUV2.x * dotRepeat2), abs(fract(gridUV2.x * dotRepeat2) - 0.5));
    }
  }
  ...
  float scanZMax = 2.0, widthScale = max(0.1, uScanGlow);
  float sigma = max(0.001, 0.18 * widthScale * uScanSoftness), sigmaA = sigma * 2.0;
  ...
  float scanZ = phase * scanZMax, dz = abs(hit.z - scanZ);
  float lineBand = exp(-0.5 * (dz * dz) / (sigma * sigma));
  float taper = clamp(uPhaseTaper, 0.0, 0.49);
  float headFade = smoother01(0.0, taper, phase), tailFade = 1.0 - smoother01(1.0 - taper, 1.0, phase);
  ...
  for (int i = 0; i < MAX_SCANS; i++) {
    ...
    float scanZI = phaseI * scanZMax, dzI = abs(hit.z - scanZI);
    float lineBandI = exp(-0.5 * (dzI * dzI) / (sigma * sigma));
    float headFadeI = smoother01(0.0, taper, phaseI), tailFadeI = 1.0 - smoother01(1.0 - taper, 1.0, phaseI);
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof window !== 'undefined' && window.DeviceOrientationEvent && (DeviceOrientationEvent as any).requestPermission) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      last = now;
      lookCurrent.current.lerp(lookTarget.current, 1 - Math.exp(-dt / smoothTime));
      tiltCurrent.current += (tiltTarget.current - tiltCurrent.current) * (1 - Math.exp(-dt / smoothTime));
      yawCurrent.current += (yawTarget.current - yawCurrent.current) * (1 - Math.exp(-dt / smoothTime));
  ...
  useEffect(() => {
    ...
    if (m) {
      ...
      u.uLinesColor.value = new THREE.Color(linesColor);
      u.uScanColor.value = new THREE.Color(scanColor);
  ...
  useEffect(() => {
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (res && res.detection) {
              const det = res.detection, box = det.box, vw = video!.videoWidth || 1, vh = video!.videoHeight || 1;
    ...
    return () => { stop = true; if (video) { const s = video.srcObject; if (s) (s as MediaStream).getTracks().forEach(t => t.stop()); video.pause(); video.srcObject = null; } };
