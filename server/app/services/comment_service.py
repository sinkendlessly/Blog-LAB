...
class CommentService:
    ...
    # ============ 列表（树形） ============
    async def list_tree(
        self, article_id: int, sort: str = "latest", current_user_id: Optional[int] = None
    ) -> List[CommentOut]:
        """获取文章评论树。"""
        # 1. 加载所有评论
        ...
        all_comments: List[Comment] = list(all_rows.scalars().all())
        if not all_comments:
            return []
        ...
        # 6. 构建树
        for c in all_comments:
            if c.parent_id is not None and c.parent_id in out_map:
                out_map[c.parent_id].replies.append(out_map[c.id])

        # 7. 排序
        roots = [out_map[c.id] for c in all_comments if c.parent_id is None]
        if sort == "oldest":
            roots.sort(key=lambda o: o.created_at)
        elif sort == "hot":
            roots.sort(key=lambda o: -len(o.replies))
        else:
            roots.sort(key=lambda o: o.created_at, reverse=True)
        return roots
